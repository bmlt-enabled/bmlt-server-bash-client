...
ADD bmlt-cli.bash-completion /etc/bash-completion.d/bmlt-cli
RUN chmod 755 /usr/bin/bmlt-cli
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/bmlt-cli\n\
' >> ~/.bashrc

#
# Setup prompt
#
RUN echo 'export PS1="[BMLT] \$ "' >> ~/.bashrc
RUN echo 'export PROMPT="[BMLT] \$ "' >> ~/.zshrc
...
line REST client for $(tput setaf 6)BMLT$(tput sgr0).\n\
\n\
